...
public record ManhuntSettings(
        ...
        @Describe("What a compass does on its own, before any Hunter has touched it. NEAREST: the "
                + "needle swings to whichever Runner is closest. CHOSEN: the same, until a Hunter "
                + "picks somebody — then it stays on them until they leave the hunt. Whether a "
                + "Hunter may pick at all is the setting below, not this one.")
        @Key("tracker-targets")
        ...
        @In("manhunt/tracker") @Title("Hunters choose for themselves who to follow")
        @Describe("On: a Hunter right-clicks their compass to step through the Runners and back to "
                + "'whoever is nearest', so each of them decides what their own needle does. Off: "
                + "nobody may change anything and every compass does whatever tracker-targets "
                + "says, which is the one way to run a hunt where the Hunters cannot single a "
                + "Runner out.")
        @Key("tracker-hunter-may-choose")
        ...
        @In("manhunt/rules") @Title("The locator bar while a hunt runs")
        @Describe("Off by default, and the one borrowed rule whose default is not UNCHANGED: the "
                + "locator bar puts every player's direction on everybody's screen, which hands the "
                + "Hunters the Runners for free and is the whole hunt given away. UNCHANGED leaves "
                + "whatever the world already had, for a server that wants it that way.")
        @Key("locator-bar-during-hunt")
        RuleOverride locatorBarDuringHunt,
        ...
        @In("manhunt/start") @Title("Start everybody in a circle")
        @Describe("On: when the hunt begins, every Runner and Hunter is placed evenly around one "
                + "circle at the hunt world's spawn, facing the middle, each side standing together. "
                + "The circle is as large as the roster needs, so neighbours always keep the same "
                + "gap. Off: everybody starts wherever they happen to be.")
        @Key("start-in-circle")
    ...
    public static final ManhuntSettings DEFAULTS = new ManhuntSettings(
            ...
            true,
            false, "", 0, 0, 0, 0, 15,
            true, 10, TrackerTargets.NEAREST, CrossWorldTracking.LAST_PORTAL, true, true, true,
            5, RunnerDeathRule.ELIMINATE, 3, true, 0,
            true, true,
            true, true, true, true,
            true, "!", true, false,
            false, RuleOverride.UNCHANGED, RuleOverride.UNCHANGED, RuleOverride.OFF,
            DifficultyOverride.UNCHANGED, true, true);
    ...
    public ManhuntSettings withRunnerWin(RunnerWinCondition runnerWin) {
        return new ManhuntSettings(
                ...
                trackerShowDistance, trackerGiveOnRespawn, trackerHunterMayChoose,
                countdownSeconds, runnerDeathRule,
                ...
                naturalRegenerationDuringHunt, locatorBarDuringHunt, difficultyDuringHunt,
                spectatorsAllowed, startInCircle);
    }
    ...
    public ManhuntSettings withLocatorBarDuringHunt(RuleOverride locatorBarDuringHunt) {
        return new ManhuntSettings(
                runnerWin, runnerAdvancementKey, hunterWin, hunterTimeoutMinutes,
                hunterReleaseDelaySeconds, resetOnStart, worldName, seedChoice, seedValue,
                closeWhitelistOnStart, chaosCooldownSeconds, runnerSelfJoinEnabled, lobbySpawnSet,
